...
import { DotsHorizontalIcon, InformationCircleIcon, VideoCameraIcon } from '@heroicons/react/outline'
...
function ChatBoxTopBar({ profile }) {
    ...
    const [_currentChatState, setCurrentChatState] = useRecoilState(currentChatState)
    ...
    return (
        ...
        <div>
            ...
            <div className="flex items-center justify-between border-b-2 px-4 py-2 h-14">
                <img className="w-8  h-8 object-cover rounded-full border p-[2px] " src={profile.photoURL} />
                ...
                </div>
                {
                    currentChatState?.id !== null ?
                        <><VideoCameraIcon onClick={() => handleVideoButton()} className="h-6 mr-4" />
