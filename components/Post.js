    ...
    return (
        <div className="bg-white my-7 border rounded-sm">
            ...
            {session && (
                ...
                <div className="flex justify-between px-4 pt-4 pb-4">
                    <div className="flex space-x-4 ">
                        ...
                        {liked ? (<HeartIconFilled onClick={likePost} className="btn text-red-500" />) : (<HeartIcon onClick={likePost} className="btn" />)}
            ...
            <p className="p-5 truncate">
                {likes.length > 0 && (
                    <div className="font-bold">{likes.length}</div>
                )}
            ...
            {session && (
                <form className="flex items-center p-4">
                    ...
                    <input onChange={(e) => setComment(e.target.value)} value={comment} type="text" placeholder="Add a comment..." className="flex-1 border-none focus:ring-0 outline-none" />
                    <button disabled={!comment.trim()} type="submit" onClick={addComment} className="font-semibold text-blue-400 disabled:text-gray-600">Post</button>
                </form>
            )}
